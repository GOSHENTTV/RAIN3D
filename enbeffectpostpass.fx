...
bool EnableStorm < string UIName = "Enable Storm Effects"; > = { true };
float ThunderIntensity < string UIName = "Thunder:: Intensity"; float UIMin = 0.0; float UIMax = 10.0; > = { 1.0 };
float ThunderGlow < string UIName = "Thunder:: Glow"; float UIMin = 0.0; float UIMax = 10.0; > = { 1.0 };
float ThunderVolume < string UIName = "Thunder:: Volume"; float UIMin = 0.0; float UIMax = 1.0; > = { 1.0 };
...
float3 lut_w(float3 res)
{
	...
	float3 thunder_day = CLutFunc(lut_thunder_day, res, thdi) * ThunderVolume;
...
bool RainRacing < string UIName = "RainDrops:: Racing"; > = { false };

VS_OUTPUT_POSTRD VS_Water_Racing(VS_INPUT_POST IN)
{
	VS_OUTPUT_POSTRD OUT;
	OUT.pos.xyz = IN.pos.xyz;
	OUT.pos.w = 1.0;
...
VS_OUTPUT_POST VS_Storm(VS_INPUT_POST IN)
{
	VS_OUTPUT_POST OUT;
	OUT.pos.xyz = IN.pos.xyz;
	OUT.pos.w = 1.0;
...
float4 PS_Storm(VS_OUTPUT_POST IN) : SV_Target
{
	float3 res = TextureColor.SampleLevel(Sampler0, IN.txcoord0.xy, 0).xyz;
	if (EnableStorm)
	{
		float thunder = sin(Timer.x * 10.0) * 0.5 + 0.5;
		thunder *= ThunderIntensity;
		res += thunder * ThunderGlow;
	}
...
technique11 Quant24
{
	pass p0
	{
		SetVertexShader(CompileShader(vs_5_0, RainRacing ? VS_Water_Racing() : VS_Water()));
...
technique11 Storm
{
	pass p0
	{
		SetVertexShader(CompileShader(vs_5_0, VS_Storm()));
		SetPixelShader(CompileShader(ps_5_0, PS_Storm()));
	}
}
